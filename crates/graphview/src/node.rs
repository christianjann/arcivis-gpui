...
impl GraphNode {
    ...
    fn render_child(child: &NodeChild, zoom: f32, text_color: Hsla, border_color: Hsla) -> Div {
        ...
        if !child.children.is_empty() {
            let children_container = div().flex().flex_wrap().gap(px(2.0 * zoom)).children(
                child
                    .children
                    .iter()
                    .map(|c| Self::render_child(c, zoom, text_color, border_color)),
            );
            ...
            Some(
                div()
                    ...
                    .children(
                        self.children
                            .iter()
                            .map(|c| Self::render_child(c, self.zoom, text_color, border_color)),
                    ),
            )
        } else {
            None
                ...
                cx.listener(|this, event: &DragMoveEvent<u64>, _window, _cx| {
                    ...
                    if let Some(offset) = this.drag_offset {
                        ...
                        let new_origin = point(
                            (event.event.position.x - this.container_offset.x - this.pan.x)
                                / this.zoom
                                - offset.x,
                            (event.event.position.y - this.container_offset.y - this.pan.y)
                                / this.zoom
                                - offset.y,
                        );
                        this.x = new_origin.x;
